...
- Git 2.26 or newer for contributor workflows; hook setup enables Git's worktree-specific configuration extension. The [Windows source-ZIP launcher](../start-harness.cmd) does not require Git.
...
The Windows launcher sets `DSH_SOURCE_ARCHIVE=1`: installation skips Git integration, and complete builds retain the package version without probing Git or inheriting commit/dirty metadata. It works in extracted ZIPs and checkouts. The root installer owns hook setup for contributor installs; pnpm disables the dependency's separate Lefthook installer while retaining other approved dependency build scripts. Official builds still require source-commit metadata.
