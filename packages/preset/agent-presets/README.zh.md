...
内置 Web `standard`、`ptc`、`cordis` preset 包含[显式文件交付](../../client/ui-deliverables/README.zh.md#explicit-deliveries)。`minimal` 提供固定 prompt，并在 POSIX 使用持久 Bash、Windows 使用一次性 cmd。`bedrock` 为 [Bedrock Failsafe](../../llm/llm-pi-ai/README.zh.md#aws-bedrock-normal-and-failsafe) 请求预算提供精简完整 prompt 和一个不支持后台作业的 shell 工具；选择它不会选择模型或更改提供方设置。
